...
std::unique_ptr<WMMM> WMMM::create (const std::string& display) {
	...
	const char* display_str = display.empty() ? NULL : display.c_str();

	xcb_connection_t *c_ = xcb_connect(display_str, NULL);
...
void WMMM::run () {
	...
	if (err == nullptr) LOG(INFO) << "yay"; else LOG(INFO) << "nay";
